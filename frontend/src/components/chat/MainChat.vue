            ...
            <v-list-item-title class="text-wrap">
              <template v-if="message.type === 'new_file' || message.message_type === 'file'">
                <template v-if="message.isImage">
                  <v-img v-if="message.file_url" :src="message.file_url" class="rounded-lg mb-2 ml-auto" max-height="200" max-width="200" contain />
                  <v-btn v-if="message.file_url || message.file_s3url" :href="message.file_url || message.file_s3url" target="_blank" download color="primary" variant="tonal" size="small">
                    <v-icon start>mdi-download</v-icon> {{ message.file_name || 'Download File' }}
                  </v-btn>
        ...
        <PartnerBubble v-else style="scroll-margin: 50px;" class="ml-2 partner-msg" :id="message.message_guid"
          ...
          <v-list-item class="py-2 my-3 ml-2 text-left">
            <v-list-item-title class="text-wrap">
              <template v-if="message.type === 'new_file' || message.message_type === 'file'">
                <template v-if="message.isImage">
                  <v-img v-if="message.file_url" :src="message.file_url" class="rounded-lg mb-2" max-height="200" max-width="200" contain />
                  <v-btn v-if="message.file_url || message.file_s3url" :href="message.file_url || message.file_s3url" target="_blank" download color="primary" variant="tonal" size="small">
                    <v-icon start>mdi-download</v-icon> {{ message.file_name || 'Download File' }}
                  </v-btn>
                  <p v-else class="text-red">Image URL missing</p>
