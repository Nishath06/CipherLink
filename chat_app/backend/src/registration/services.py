...
async def create_user(db_session: AsyncSession, *, user_schema: UserRegisterSchema) -> User:
    ...
    await db_session.flush()  # Flush to generate the GUID before adding keys

    # ✅ Generate ECC key pair
    private_key, public_key = generate_ecc_key_pair()

    # ✅ Store ECC keys in the table
    ecc_key = ECCKey(
        user_guid=new_user.guid,  # Link ECC keys to the newly created user
