...
from sqlalchemy import Boolean, Column, DateTime, Enum, ForeignKey, Index, String, Table, LargeBinary, func

from sqlalchemy.dialects.postgresql import JSONB, UUID
from sqlalchemy.orm import Mapped, mapped_column, relationship

from src.database import BaseModel, RemoveBaseFieldsMixin, metadata,Base
...
class User(BaseModel):
    ...
    ecc_keys: Mapped[List["ECCKey"]] = relationship("ECCKey", back_populates="user")
...
class Message(BaseModel):
    ...
    ecc_key_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("ecc_keys.id"), nullable=True)

    # Define relationships
    ecc_key: Mapped["ECCKey"] = relationship("ECCKey", back_populates="messages")
    chat: Mapped["Chat"] = relationship(back_populates="messages")
    user: Mapped["User"] = relationship(back_populates="messages")
    ecc_keys: Mapped[List["ECCKey"]] = relationship("ECCKey", back_populates="messages")
...
class ECCKey(BaseModel):  # Inheriting from BaseModel
    """Table to store ECC keys"""
    __tablename__ = "ecc_keys"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, index=True, default=uuid.uuid4)
    user_guid: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("user.guid", ondelete="CASCADE"), nullable=False, unique=True)
    public_key: Mapped[str] = mapped_column(String(1024), nullable=False)
    private_key: Mapped[str] = mapped_column(String(1024), nullable=False)

    # Relationship to user
    user: Mapped["User"] = relationship("User", back_populates="ecc_keys")
    messages: Mapped[List["Message"]] = relationship("Message", back_populates="ecc_key")
    # If you want to add timestamp fields, you can inherit that behavior from BaseModel, assuming it's set up in BaseModel.

class EncryptedFile(BaseModel):
    """Table to store encrypted file details"""
    __tablename__ = "encrypted_files"

    id = Column(UUID(as_uuid=True), primary_key=True, index=True, default=uuid.uuid4)
    filename = Column(String(255), nullable=False)
    user_guid = Column(UUID(as_uuid=True), ForeignKey("user.guid"), nullable=False)
